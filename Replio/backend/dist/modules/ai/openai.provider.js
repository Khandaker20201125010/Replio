...
class OpenAIProvider {
    constructor() {
        ...
        if (env_1.env.OPENAI_API_KEY) {
            ...
            logger_1.logger.warn("OpenAI API key not configured, using fallback behavior");
        }
    }
    analyzeComment(comment, context) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            if (!this.client) {
                throw new errors_1.ExternalAPIError("OpenAI client not configured", "openai");
            }
            try {
...
Respond with valid JSON only, no other text.`,
                ...
                if (!content) {
                    throw new errors_1.ExternalAPIError("No response from OpenAI", "openai");
                }
                // Extract JSON from response (in case there's extra text)
                const jsonMatch = content.match(/\{[\s\S]*\}/);
                if (!jsonMatch) {
                    throw new errors_1.ExternalAPIError("Could not extract JSON from OpenAI response", "openai");
                }
                ...
                if (!result.intent ||
                    ...
                    throw new errors_1.ExternalAPIError("Invalid AI response structure", "openai");
                }
                return result;
            }
            catch (error) {
                logger_1.logger.error({ error }, "OpenAI analysis failed");
                throw new errors_1.ExternalAPIError("Failed to analyze comment with OpenAI", "openai");
            }
        });
    }
...
Respond with the reply text only, no other text.`,
                ...
                throw new errors_1.ExternalAPIError("Failed to generate reply with OpenAI", "openai");
            }
        });
    }
    validateReply(reply) {
